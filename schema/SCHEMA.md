...
`return_length` records a named, reusable exception to the ordinary return length. `{"kind": "shortest_of", "params": ["x", "y"]}` bounds a result by every listed input. `{"kind": "recycled_values", ...}` describes vector recycling without treating controls as values: `value_params` identifies recycled inputs (currently `...`), `control_params` identifies excluded controls, `all_values_zero: "zero"` preserves the all-empty result, `collapse` says a non-NULL control reduces the output to a stated length, and `recycle0` says a true control makes any empty value input yield zero. This models the real `paste`/`paste0` contract, including `collapse` and `recycle0`, without special-casing function names.
...
`predicate` declares that a scalar logical result tests `subject_param` against an R type `target`, allowing normal branch narrowing from ordinary call resolution rather than a name table. `rlang::is_null` is the canonical null-predicate example.

`assertion` declares an assertion-style helper that, when it returns normally, proves `target` for `subject_param`. It may name `allow_null_param` and `allow_na_param` for conservative weakening. To avoid claiming semantics for conventional helper names, assertions are accepted only with real provenance: `provenance` must be `{"kind": "standalone_types_check", "fingerprint_params": ["arg", "call"]}` and those parameters must exist in the signature. This is intentionally restricted to rlang's standalone `check_*` implementation shape; generic assertion metadata is not inferred from a name.
