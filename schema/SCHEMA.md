...
`injection` maps formal names to `"full"` (tidy-evaluation `!!` and `!!!`) or `"splice"` (dynamic dots accepting only `!!!`). For example, `rlang::list2()` declares `{"...": "splice"}`. It uses ordinary argument matching, including `...`. Data masking alone does not imply injection: base `with()` evaluates `!!x` as two negations. Expressions outside an opted-in argument retain ordinary R semantics.
