...
Each JSON file describes one R package. Validate stubs with `ry typeshed validate`; the serde definitions and loader in ry's `crates/ry-typeshed/src/lib.rs` are the normative machine-readable schema.

The required header fields are `schema_version`, `package`, and `version`. `functions` is also required; `globals`, `datasets`, and `s3_methods` are optional. Schema version 2 extends version 1 with declarative predicate, assertion, conditional scope, and return-length semantics. ry versions that support only version 1 must reject version 2 instead of ignoring its semantic fields.
...
A parameter may be a bare name string or an object. A bare string means an
untyped, optional parameter. Parameter objects have these fields:

| Field | Meaning |
| --- | --- |
| `name` | Required parameter name. |
| `type` | Optional [R type object](#return-types). If absent, the checker does not check the parameter's type. |
| `required` | Whether the call must bind the parameter. Defaults to `false`. |
| `default` | Records whether the R formal has a default. Informational only. |

The variadic `...` parameter may be a string or `{"name": "..."}`, but cannot
be typed or required.
...
`na` is an NA-capability claim on an R type object: `true` declares the value may be `NA`, `false` declares it never is, and an absent field declares neither way — absence is not a non-NA guarantee. Opaque entries with unknown missingness record `na: true` uniformly so inventories stay shape-consistent and the claim stays a conservative upper bound (as in the ggplot2, grid, vctrs, set6, and dictionar6 inventories); `na: false` is reserved for values known never to be `NA`, such as the htmltools/shiny `tags` lists.
...
`higher_order` declares callback invocation and result semantics. `callback_param` and zero-based `callback_position` must identify the same formal parameter in `params`. `callback_args` may contain `element_of_arg0`, `element_of_arg1`, `unknown`, or `accumulator_and_element`; `elements_after_callback` represents variadic `Map`-style calls. Result kinds are `list_of_callback_return`, `vector_of`, `same_as_arg0`, `callback_return`, `first_arg`, `simplify`, `fun_value_template`, and `callback_identity`. For `vector_of`, `mode: "opaque"` means the complete result is unknown: it promises neither an atomic vector nor a known length, class, missingness, or element schema. Callback invocation metadata remains independent of this result uncertainty. A result may additionally name its vector `mode`, a `length_arg`, a returned `source_arg`, the `template_position`, whether its length becomes unknown, and whether a list result retains callback element schema. These optional properties preserve package-specific call shapes without encoding function names in the checker. For folds (`accumulator_and_element`), `callback_return` describes the repeated update, not an unconditional final return. Consumers must keep the result unknown unless they can prove the iteration count, initializer behavior, and accumulation mode. They must not borrow the input type, initializer type, or one callback invocation’s type as the final result: zero or singleton inputs can bypass the callback, and later accumulators can change shape.
