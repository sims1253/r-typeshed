...
`return_length` records a named, reusable exact-length rule. It is not an interval or upper-bound language: consumers whose length domain cannot retain a fact must yield `unknown`, never turn a bound into an exact length. `{"kind": "zero_if_any_param_zero", "params": ["x", "y"]}` returns exact zero when any listed, bound parameter is known to have exact length zero; in every other case it yields unknown. This captures the only exact length fact supplied by a set-operation bound such as `intersect()` without falsely claiming that its result has the shorter input's length.

`{"kind": "recycled_values", ...}` describes vector recycling without treating controls as values. `value_params` and `control_params` are disjoint formal-parameter names, resolved with R's normal exact, partial, and positional argument matching. A `...` value parameter denotes every argument captured by that formal; it does not include arguments bound to a named control. All non-`...` value parameters and every control must be explicit, so a consumer can determine the value set without relying on an argument's source position. `all_values_zero: "zero"` preserves the all-empty result, `collapse` says a non-NULL control reduces the output to a stated length, and `recycle0` says a true control makes any empty value input yield zero. This models the real `paste`/`paste0` contract, including `collapse` and `recycle0`, without special-casing function names.
